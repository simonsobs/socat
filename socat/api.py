...
from .database import ExtragalacticSource, get_async_session, async_engine
...
async def create_source(model: SourceModificationRequest, session: SessionDependency) -> ExtragalacticSource:
    if model.ra is None or model.dec is None:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="RA and Dec must be provided")
...
async def update_source(source_id: int,  model: SourceModificationRequest, session: SessionDependency) -> ExtragalacticSource:
    try:
        response = await core.update_source(source_id, model.ra, model.dec, session=session)
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))
